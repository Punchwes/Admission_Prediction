...
#df2=df2.drop('EnglishLevel',axis=1)
#df2=df2.drop('Term',axis=1)
#df2=df2.drop('AnnouncedTime',axis=1)
...
#observe_decline=observe[observe['Result']==0]
#observe_offer=observe[observe['Result']==1]
#observe_decline_test=observe_decline.sample(frac=0.25)
#observe_offer_test=observe_offer.sample(observe_decline_test.count())
#observe_test=pd.concat([observe_decline_test,observe_offer_test])
#observe_train=observe-observe_test
...
#data_train,data_test,target_train,target_test=train_test_split(observe_data,labels,test_size=0.25,stratify=labels)
...
#
#data_train_df=pd.DataFrame(data_train,columns=observe_data.columns)
...
estimators['xgboost']= xgb.XGBClassifier(subsample=0.8,colsample_bytree=0.8,learning_rate =0.1,max_depth=30,objective='binary:logistic',nthread=-1,
...
#    scores = cross_validation.cross_val_score(estimators[k], observe, labels, cv=10,scoring='average_precision')
...
#    
    ...
    print ('###########%s 权重为#############'%k)
    imp=list(zip(observe_data.columns, estimators[k].feature_importances_))
...
#    Term_imp=impp[impp['features']=='Term']
#    AnnouncedTime_imp=impp[impp['features']=='PostGPA']
...
#    cal14=Term_imp['importance']
#    cal15=AnnouncedTime_imp['importance']
...
#    test=pd.DataFrame({'AppliedSchool':'爱丁堡大学','Degree':'硕士','Major':'Artificial Intelligence','SchoolMajor':'计算机科学与技术','GRE':325,'EnglishLevel':1,'School':'211 & 985','GPA':80},index=[0])
...
#test2=test1.reindex(columns=observe_data.columns,fill_value=0)
